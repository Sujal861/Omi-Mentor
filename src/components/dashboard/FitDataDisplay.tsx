...
export const FitDataDisplay = ({ fitData, isLoading, onRefresh, onConnect }: FitDataDisplayProps) => {
  ...
  if (!fitData) {
    return (
      <ThreeDCard className="rounded-lg overflow-hidden">
        <Card>
          <CardHeader>
...
const StatsCard = ({ icon, title, value, color }: StatsCardProps) => {
  return (
    <ThreeDCard className="rounded-lg overflow-hidden" rotationIntensity={5}>
      <div className="bg-card rounded-lg border p-3 flex flex-col h-full">
